...
export default function AccountSettingsPage() {
  ...
  const [toggles, setToggles] = useState({
    emailNotifications: true, smsAlerts: false, twoFactor: false, pushNotifications: true,
  });
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between"
        style={{ padding: '16px 20px', borderBottom: '1px solid #e8e8e8', background: '#fff' }}>
        <div className="flex items-center gap-3">
          ...
          <button type="button" onClick={() => setMenuOpen(!menuOpen)}
            ...
            <motion.span animate={{ rotate: menuOpen ? 45 : 0, y: menuOpen ? 7 : 0 }}
              style={{ display: 'block', width: '20px', height: '2px', background: '#111', borderRadius: '1px' }} />
            <motion.span animate={{ opacity: menuOpen ? 0 : 1 }}
              style={{ display: 'block', width: '20px', height: '2px', background: '#111', borderRadius: '1px' }} />
            <motion.span animate={{ rotate: menuOpen ? -45 : 0, y: menuOpen ? -7 : 0 }}
              style={{ display: 'block', width: '20px', height: '2px', background: '#111', borderRadius: '1px' }} />
          </button>
          <p style={{ fontSize: '18px', fontWeight: 700, color: '#6C2BD9', fontFamily: 'Outfit, sans-serif' }}>PopX</p>
        </div>
        <div className="flex items-center gap-3">
          {saved && <span style={{ fontSize: '12px', color: '#22C55E', fontWeight: 500 }}>Saved!</span>}
          <motion.button whileTap={{ scale: 0.97 }} onClick={handleSave}
            className="cursor-pointer border-none text-white"
            style={{ padding: '8px 16px', background: '#6C2BD9', borderRadius: '8px', fontSize: '13px', fontWeight: 600 }}>
            Save
          </motion.button>
        </div>
      </div>
        ...
        {menuOpen && (
          <>
            ...
            <motion.div
              ...
              style={{ position: 'fixed', top: 0, left: 0, bottom: 0, width: '280px', background: '#fff', zIndex: 50, padding: '24px 16px', boxShadow: '4px 0 24px rgba(0,0,0,0.1)' }}>

              {/* Menu Header */}
              <div className="flex items-center gap-3" style={{ marginBottom: '24px', padding: '0 8px' }}>
                <motion.img src="https://i.pravatar.cc/150?img=47" alt="Profile"
                  className="rounded-full object-cover" style={{ width: '44px', height: '44px' }}
                  initial={{ scale: 0.8 }} animate={{ scale: 1 }} />
                <div style={{ overflow: 'hidden' }}>
                  <p style={{ fontSize: '15px', fontWeight: 600, color: '#111', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{user.fullName}</p>
                  <p style={{ fontSize: '12px', color: '#888', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{user.email}</p>
                </div>
              </div>

              <div style={{ borderBottom: '1px solid #e8e8e8', marginBottom: '12px' }} />

              {/* Nav Items */}
              <nav className="flex flex-col gap-1 flex-1">
                {navItems.map((item) => (
                  <button key={item.key} type="button" onClick={() => selectSection(item.key)}
              ...
              <div style={{ borderTop: '1px solid #e8e8e8', paddingTop: '12px' }}>
                <button type="button" onClick={handleLogout}
                  ...
                  style={{ padding: '14px 12px', borderRadius: '10px', background: 'transparent', color: '#EF4444', fontSize: '15px', fontWeight: 500 }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div style={{ padding: '20px 20px 8px' }}>
        <p style={{ fontSize: '12px', color: '#888', textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: 600 }}>
      ...
      <div style={{ padding: '0 20px 24px' }}>
        ...
        {activeSection === 'profile' && (
          <motion.div key="profile" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <div className="flex items-center gap-4">
                <div className="relative flex-shrink-0">
                  <motion.img src="https://i.pravatar.cc/150?img=47" alt="Profile"
                    ...
                    style={{ width: '64px', height: '64px', outline: '3px solid #6C2BD9', outlineOffset: '2px' }}
                    initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} />
                  <div className="absolute flex items-center justify-center rounded-full"
                    style={{ width: '24px', height: '24px', background: '#6C2BD9', bottom: '0', right: '0', border: '2px solid #fff' }}>
                ...
                <div>
                  <p style={{ fontSize: '16px', fontWeight: 600, color: '#111' }}>{user.fullName}</p>
                  <p style={{ fontSize: '13px', color: '#888', marginTop: '2px' }}>{user.email}</p>
                  <p style={{ fontSize: '11px', color: '#6C2BD9', marginTop: '2px', fontWeight: 500 }}>{user.isAgency ? 'Agency' : 'Creator'}</p>
                </div>
              </div>
            </div>
            ...
            <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '20px' }}>
              <p style={{ fontSize: '14px', fontWeight: 600, color: '#111', marginBottom: '12px' }}>Account Type</p>
              <div className="flex gap-3">
                {(['Agency', 'Creator'] as const).map((option) => {
                  const selected = option === 'Agency' ? isAgency : !isAgency;
                  return (
                    <button key={option} type="button" onClick={() => setIsAgency(option === 'Agency')} className="cursor-pointer flex-1"
                      style={{ padding: '10px', borderRadius: '8px', border: `1px solid ${selected ? '#6C2BD9' : '#e0e0e0'}`, background: selected ? '#f0ecf9' : '#fff', color: selected ? '#6C2BD9' : '#888', fontSize: '14px', fontWeight: 500, transition: 'all 0.2s' }}>
                      {option}
                    </button>
                  );
                })}
        ...
        {activeSection === 'security' && (
          <motion.div key="security" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '20px', marginBottom: '16px' }}>
              <h2 style={{ fontSize: '16px', fontWeight: 600, color: '#111', marginBottom: '16px' }}>Change Password</h2>
              ...
              <motion.button whileTap={{ scale: 0.97 }} className="w-full cursor-pointer border-none text-white"
                style={{ padding: '12px', background: '#6C2BD9', borderRadius: '8px', fontSize: '14px', fontWeight: 600 }}>
                Update Password
              </motion.button>
            </div>

            <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '20px' }}>
              <h2 style={{ fontSize: '16px', fontWeight: 600, color: '#111', marginBottom: '8px' }}>Two-Factor Authentication</h2>
              <ToggleSwitch label="Enable 2FA" description="Add an extra layer of security using an authenticator app"
        ...
        {activeSection === 'notifications' && (
          <motion.div key="notifications" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '20px' }}>
              <h2 style={{ fontSize: '16px', fontWeight: 600, color: '#111', marginBottom: '4px' }}>Notification Preferences</h2>
              <p style={{ fontSize: '13px', color: '#888', marginBottom: '8px' }}>Choose how you want to be notified.</p>
              <ToggleSwitch label="Email Notifications" description="Receive updates about campaigns and messages" enabled={toggles.emailNotifications} onToggle={() => toggleOption('emailNotifications')} />
              <div style={{ borderBottom: '1px solid #e8e8e8' }} />
              ...
              <div style={{ borderBottom: '1px solid #e8e8e8' }} />
              <ToggleSwitch label="Push Notifications" description="Receive push notifications in your browser" enabled={toggles.pushNotifications} onToggle={() => toggleOption('pushNotifications')} />
            </div>
          </motion.div>
        )}
        ...
        {activeSection === 'billing' && (
          <motion.div key="billing" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '20px', marginBottom: '16px' }}>
              <h2 style={{ fontSize: '16px', fontWeight: 600, color: '#111', marginBottom: '12px' }}>Current Plan</h2>
              <div style={{ padding: '16px', background: '#f0ecf9', borderRadius: '10px', marginBottom: '16px' }}>
                <p style={{ fontSize: '16px', fontWeight: 600, color: '#6C2BD9' }}>Free Plan</p>
                <p style={{ fontSize: '12px', color: '#888', marginTop: '4px' }}>Basic features for individual creators</p>
                <motion.button whileTap={{ scale: 0.97 }} className="cursor-pointer border-none text-white"
                  style={{ marginTop: '12px', padding: '10px 20px', background: '#6C2BD9', borderRadius: '8px', fontSize: '13px', fontWeight: 600 }}>
                  Upgrade
                </motion.button>
              </div>
              <div style={{ borderBottom: '1px solid #e8e8e8', marginBottom: '16px' }} />
              <h3 style={{ fontSize: '14px', fontWeight: 600, color: '#111', marginBottom: '12px' }}>Usage This Month</h3>
              <div className="flex flex-col gap-3">
                {[{ label: 'Campaigns', value: '2 / 5' }, { label: 'Storage', value: '120MB / 1GB' }, { label: 'API Calls', value: '450 / 1000' }].map((item) => (
                  <div key={item.label} className="flex items-center justify-between" style={{ padding: '8px 0' }}>
                    <p style={{ fontSize: '13px', color: '#888' }}>{item.label}</p>
            ...
            <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '20px' }}>
              <h2 style={{ fontSize: '16px', fontWeight: 600, color: '#111', marginBottom: '8px' }}>Payment Method</h2>
              <p style={{ fontSize: '13px', color: '#888', marginBottom: '16px' }}>No payment method added yet.</p>
              <motion.button whileTap={{ scale: 0.97 }} className="w-full cursor-pointer text-white border-none"
                style={{ padding: '12px', background: '#6C2BD9', borderRadius: '8px', fontSize: '13px', fontWeight: 600 }}>
                Add Payment Method
              </motion.button>
            </div>
          </motion.div>
        ...
        {activeSection === 'help' && (
          <motion.div key="help" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '20px', marginBottom: '16px' }}>
              <h2 style={{ fontSize: '16px', fontWeight: 600, color: '#111', marginBottom: '12px' }}>FAQ</h2>
              {[
                ...
                { q: 'How do I connect with creators?', a: 'Browse the Creator Marketplace to find creators that match your niche. Send collaboration requests directly through the platform.' },
                { q: 'What payment methods are accepted?', a: 'We accept all major credit cards (Visa, Mastercard, Amex), bank transfers, and PayPal. All payments are processed securely.' },
                { q: 'How can I track performance?', a: 'Visit the Analytics section to see real-time metrics for all your campaigns including reach, engagement, conversions, and ROI.' },
              ].map((faq) => (
                <details key={faq.q} style={{ borderBottom: '1px solid #e8e8e8', padding: '12px 0', cursor: 'pointer' }}>
            ...
            <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '20px' }}>
              <h2 style={{ fontSize: '16px', fontWeight: 600, color: '#111', marginBottom: '8px' }}>Contact Support</h2>
              <p style={{ fontSize: '13px', color: '#888', marginBottom: '16px' }}>Our team is available Mon-Fri, 9am-6pm.</p>
              <div className="flex flex-col gap-3">
                <motion.button whileTap={{ scale: 0.97 }} className="w-full cursor-pointer text-white border-none"
                  style={{ padding: '12px', background: '#6C2BD9', borderRadius: '8px', fontSize: '13px', fontWeight: 600 }}>
                  Email Support
                </motion.button>
                <motion.button whileTap={{ scale: 0.97 }} className="w-full cursor-pointer"
                  style={{ padding: '12px', background: '#fff', border: '1px solid #6C2BD9', borderRadius: '8px', fontSize: '13px', fontWeight: 600, color: '#6C2BD9' }}>
                  Live Chat
                </motion.button>
              </div>
            </div>
