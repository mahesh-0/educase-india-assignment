...
export default function LoginPage() {
  ...
  return (
    ...
    >
      <h1 style={{ fontSize: '26px', fontWeight: 700, color: '#111', lineHeight: 1.3, marginBottom: '10px' }}>
      ...
      <div style={{ marginTop: '8px' }}>
        <motion.button
          whileTap={{ scale: 0.97 }} onClick={handleLogin}
          className="w-full cursor-pointer border-none text-white"
          style={{
            height: '52px', background: isFormValid ? '#6C2BD9' : '#C4C4C4',
            fontWeight: 600, fontSize: '16px', borderRadius: '8px', transition: 'background 0.2s',
          }}
        >
          Login
        </motion.button>
      ...
      <p style={{ fontSize: '13px', color: '#888', textAlign: 'center', marginTop: '20px' }}>
        ...
        <button type="button" onClick={() => navigate('/signup')} className="bg-transparent border-none cursor-pointer p-0"
          style={{ color: '#6C2BD9', fontWeight: 600, fontSize: '13px' }}>Create one</button>
      </p>
    </motion.div>
  );
}
